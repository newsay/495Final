...
if (authorize($auth_service, $organization_service, User::USER_TYPE_EMPLOYEE)) {
    ...
    if ($auth_service->get_user_organization() != $shift->get_organization_id()) {
        ...
        if ($shift->get_status() == Shift::STATUS_UNASSIGNED) {
            echo '<h2 class="text-center">--- Request Submitted ---</h2>';
            echo '<h3 class="text-center">--- You Can Track Your Request On Your Dashboard ---</h3>';

            // Signs the user up for the shift                  
            $request_service->submit_request(Request::REQUEST_ASSIGNMENT, $auth_service->get_user_id(), $_GET["requestshiftid"]);
        } else {
